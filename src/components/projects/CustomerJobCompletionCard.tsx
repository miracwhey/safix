...
function CustomerJobCompletionCardView({ vm }: { vm: CompletedWorkProofViewModel }) {
  const isRefund = !vm.isFullyPaid
  // While a dispute decision is still settling, the money has NOT moved yet —
  // render an in-progress (amber) state and "…wird verarbeitet" copy, never a
  // terminal "Betrag erstattet / Zahlung freigegeben".
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-[14px] leading-relaxed text-slate-500">
        {isSettling
          ? vm.settlementIsRefund
            ? 'Das Projekt wurde abgeschlossen. Die Rückerstattung im Rahmen der Streitbeilegung wird gerade verarbeitet.'
            : 'Das Projekt wurde abgeschlossen. Die Auszahlung an den Handwerker wird gerade verarbeitet.'
          : vm.isFullyPaid
            ? 'Du hast die Zahlung freigegeben. Das Projekt ist damit offiziell abgeschlossen. Vielen Dank für dein Vertrauen.'
            : 'Das Projekt wurde abgeschlossen. Der Betrag wurde im Rahmen der Streitbeilegung erstattet.'}
      </p>
      ...
      <div className="space-y-2.5">
        {vm.totalAmount > 0 && (
          <DetailRow
            label={isSettling ? 'Betrag' : vm.isFullyPaid ? 'Freigegebener Betrag' : 'Erstatteter Betrag'}
        ...
        <DetailRow
          ...
          value={formatDate(vm.paymentReleasedAt)}
        />
      </div>
      ...
      <div className={`mt-4 flex items-center gap-2 rounded-2xl px-4 py-3 ${statusBannerBg}`}>
        ...
        <span className={`text-[13px] font-semibold ${statusBannerText}`}>
          {isSettling
            ? vm.settlementIsRefund
              ? 'Projekt geschlossen · Erstattung wird verarbeitet'
              : 'Projekt geschlossen · Auszahlung wird verarbeitet'
            : vm.isFullyPaid
              ? 'Zahlung freigegeben · Projekt geschlossen'
              : 'Projekt geschlossen · Betrag erstattet'}
        </span>
      </div>
    </section>
  )
...
export default function CustomerJobCompletionCard({ jobId }: Props) {
  function buildVm(): { vm: CompletedWorkProofViewModel; craftsmanUserId?: string } | null {
    if (!jobId) return null
    // Hydration gate (Z.125): settlement truth lives on the dispute and the VM
    // derives `isFullyPaid` from payment.state — so BOTH repos must be hydrated.
    // On a cold load where the dispute is hydrated but payment is not,
    // getPaymentForJob returns undefined and paymentState falls back to the stale
    // job.paymentState mirror → isFullyPaid mis-derived. Until both are hydrated
    // we never render a terminal "Betrag erstattet / Zahlung freigegeben" from
    // not-yet-loaded data.
    ...
    const vm = deriveCompletedWorkProof(job, payment ?? undefined, artifacts, feedback, dispute)
    if (!vm) return null
    return { vm, craftsmanUserId: job.craftsmanUserId }
  }

  const [result, setResult] = useState<{ vm: CompletedWorkProofViewModel; craftsmanUserId?: string } | null>(buildVm)
  ...
  const { vm, craftsmanUserId } = result

  return (
    <div className="space-y-4">
      <CustomerJobCompletionCardView vm={vm} />
      {/* Only invite feedback once the happy release path has actually settled —
          never while a dispute-driven payout/refund is still in progress. */}
      {vm.isFullyPaid && !vm.settlementPending && jobId && craftsmanUserId ? (
        <CustomerFeedbackCard jobId={jobId} craftsmanUserId={craftsmanUserId} />
      ) : null}
    </div>
  )
}
