...
-- ---------------------------------------------------------------------------
-- 1. BEFORE-UPDATE FSM + authority trigger
-- ---------------------------------------------------------------------------
-- SECURITY DEFINER (owner postgres): it must read public.providers reliably to
-- detect the provider-side payee even when RLS would hide that row (an
-- RLS-hidden provider row would silently defeat the payee block — NULL/RLS
-- poisoning). auth.uid() and current_setting() read the request GUCs and stay
-- caller-scoped under SECURITY DEFINER, which is exactly what we need.
...
BEGIN
  ...
  -- (2) trusted SECDEF RPC sentinel (finalize_payment_state_atomic et al.).
  ...
  -- (4) FSM transition guard — enforced ONLY within the canonical escrow set.
  --     If either side is a legacy CHECK-only state, do NOT hard-block (keeps
  --     legacy Stripe-ish writes — authorized/captured/escrowed/etc. — working).
  IF public.payment_fsm_is_canonical(v_old)
    ...
    RAISE EXCEPTION
      ...
      -- → flushPendingMutations DROPS the queue entry. P0001 would classify as
      -- 'unknown' → conservative RETRY-loop on a permanently-illegal write.
      -- Mirrors jobs_terminal_status_guard's errcode choice.
      USING ERRCODE = '23514';
  END IF;

  -- (5) AUTHORITY guard: the PAYEE may never drive money to released/refunded.
  --     Ownership read from OLD; every compare NULL-guarded.
  IF v_new IN ('released', 'refunded') THEN
    v_is_payee :=
         (OLD.craftsman_user_id IS NOT NULL AND OLD.craftsman_user_id = v_uid)
      OR (OLD.provider_id IS NOT NULL AND EXISTS (
            SELECT 1
              FROM public.providers pr
             WHERE pr.id = OLD.provider_id
               AND pr.profile_id IS NOT NULL
               AND pr.profile_id = v_uid));

    IF v_is_payee THEN
      RAISE EXCEPTION
        'payment_authority_denied: the payee may not drive payment % to ''%'' — use the release/refund RPC or server path',
        OLD.id, v_new
        USING ERRCODE = '42501';
    END IF;
  END IF;
...
-- Owner = postgres for parity with finalize + a deterministic RLS-bypass
-- guarantee when reading public.providers under SECURITY DEFINER.
